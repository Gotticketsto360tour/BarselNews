...
```{r}
# Load the data
sentences <- read_csv("data/Sentiment Analysis/Analysis Ready/control_sentence_data.csv")
article <- read_csv("data/Sentiment Analysis/Analysis Ready/control_article_data.csv")
# How many articles for each group
...
```{r}
# Load the data
sentences <- read_csv("data/Sentiment Analysis/Analysis Ready/comparaison_sentence_data.csv")
article <- read_csv("data/Sentiment Analysis/Analysis Ready/comparaison_article_data.csv")
# How many articles for each group
...
eg <-s  %>%
    ...
    pivot_longer(contains("Eg")) %>%
    separate(name, into=c("variable", "group", "sentiment")) %>%
    mutate(group = ifelse(group == 1, "leave reform news", "control news"))
# Plot
eg %>%
    mutate(sentiment=case_when(sentiment == 1 ~ "positive",
                                sentiment == 2 ~ "neutral",
